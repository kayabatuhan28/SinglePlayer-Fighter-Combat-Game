...
{
	Super::BeginPlay();
	SphereComponent->OnComponentBeginOverlap.AddDynamic(this,&AHeavyProjectile::BeginOverleap);
	GetWorld()->GetTimerManager().SetTimer(MyTimerHandle,this,&AHeavyProjectile::DestroyTime,4.f,false);
...
void AHeavyProjectile::BeginOverleap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor,
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if(OtherActor->ActorHasTag("Player"))
	{
		...
		if(PlayerRef )
		{
			PlayerRef->getKnockBackHit(50.f,1.f);
...
void AHeavyProjectile::DestroyTime()
{
	K2_DestroyActor();
}
